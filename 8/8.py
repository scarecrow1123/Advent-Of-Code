...
encode_chars = []
for line in lines:
  temp = line.decode("string_escape")[1:-1]
  tokens = [i for i in line] 
  ...
  for idx, token in enumerate(tokens):
    ...
    if token == "\\":
      copy[idx] = "\\\\"
  copystr = '"' + ''.join(copy) + '"'
  code_chars.append(len(line))
  mem_chars.append(len(temp))
  encode_chars.append(len(copystr))
print sum(code_chars) - sum(mem_chars)
print sum(encode_chars) - sum(code_chars)
